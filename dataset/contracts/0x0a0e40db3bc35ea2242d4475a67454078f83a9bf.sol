...
contract ReleasableToken is StandardToken, Ownable {
    ...
    function transfer(
        address to,
        uint value
    ) public canTransfer(msg.sender) returns (bool success) {
    ...
    function transferFrom(
        ...
        address to,
        uint value
    ) public canTransfer(from) returns (bool success) {
...
contract UpgradeableToken is EIP20Token, Burnable {
     ...
     */
    function changeUpgradeMaster(address new_master) public onlyMaster {
...
contract LostAndFoundToken {
     ...
     */
    function enableLostAndFound(
        address agent,
        uint tokens,
        EIP20Token token_contract
    ) public {
...
contract MintableToken is Mintable, Ownable {
    ...
    function MintableToken(
        ...
        if (initialSupply > 0) mintInternal(multisig, initialSupply);
    ...
    LostAndFoundToken
{
    ...
    function CrowdsaleToken(
        ...
        address token_retriever
    )
        public
        UpgradeableToken(team_multisig)
        MintableToken(initial_supply, team_multisig, true)
    {
...
contract GenericCrowdsale is Haltable {
    ...
    event InvestmentPolicyChanged(
        bool requireCId,
        bool requireSignedAddress,
        address signer
    );
     ...
     */
    function configurationGenericCrowdsale(
        address team_multisig,
        uint start,
        uint end
    ) internal inState(State.PendingConfiguration) {
    ...
    function investInternal(
        address receiver,
        uint128 customerId
    ) private stopInEmergency notFinished {
     ...
     */
    function buyOnBehalfWithSignedAddress(
        address receiver,
        uint128 customerId,
        uint8 v,
        bytes32 r,
        bytes32 s
    ) public payable validCustomerId(customerId) {
     ...
     * @param customerId UUIDv4 that identifies this contributor
     */
    function buyOnBehalfWithCustomerId(
        address receiver,
        uint128 customerId
    ) public payable validCustomerId(customerId) unsignedBuyAllowed {
        investInternal(receiver, customerId);
    }
     ...
     *
     */
    function setRequireSignedAddress(
        bool value,
        address signer
    ) public onlyOwner {
    ...
    function setEarlyParticipantWhitelist(
        address addr,
        bool status
    ) public onlyOwner notFinished stopInEmergency {
    ...
    function calculateTokenAmount(
        uint weiAmount,
        address receiver
    ) internal view returns (uint weiAllowed, uint tokenAmount);
    ...
    modifier notFinished() {
        ...
        require(
            current_state == State.PreFunding || current_state == State.Funding
        );
        _;
    }
...
contract TokenTranchePricing {
    ...
    function getCurrentTranche(
        uint tokensSold
    ) private view returns (Tranche storage) {
        for (uint i = 0; i < tranches.length; i++) {
            if (
                tranches[i].start <= now &&
                now < tranches[i].end &&
                tokensSold < tranches[i].amount
            ) {
                return tranches[i];
            }
        }
    ...
    TokenTranchePricing
{
    ...
    function configurationCrowdsale(
        address team_multisig,
        uint start,
        uint end,
        address token_retriever,
        uint[] init_tranches,
        uint multisig_supply,
        uint crowdsale_supply,
        uint8 token_decimals,
        uint max_tokens_to_sell
    ) public onlyOwner {
    ...
    function calculateTokenAmount(
        ...
        uint tokensPerEth = getCurrentPrice(tokensSold)
            .mul(milieurs_per_eth)
            .div(1000);
        uint maxWeiAllowed = sellable_tokens.sub(tokensSold).mul(1 ether).div(
            tokensPerEth
        );
